...
export function Button({
  ...
  const base =
    "rounded-2xl px-4 py-2 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2";
  const styles =
    variant === "primary"
...
export function Card({ className, ...props }: HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn(
        "rounded-2xl border border-[hsl(var(--grayblue))] bg-[hsl(var(--beige))] shadow-soft",
        className,
      )}
      {...props}
    />
...
export function QuoteCard({
  ...
  return (
    <Card className="text-center">
      <CardBody>
        <p className="text-lg font-semibold text-[hsl(var(--charcoal))]">
          “{quote}”
        </p>
        {author ? (
          <p className="mt-2 text-sm text-[hsl(var(--charcoal))]/70">
            — {author}
          </p>
        ) : null}
      </CardBody>
...
export function MoodSelector({ onSelect }: { onSelect?: (n: number) => void }) {
  ...
  return (
    <Card>
      <CardHeader>
        <h3 className="text-sm font-semibold text-[hsl(var(--charcoal))]">
          Quick Mood
        </h3>
      </CardHeader>
  ...
  return (
    <span
      className={cn(
        ...
        locked
          ? "bg-white text-[hsl(var(--charcoal))]/60"
...
export function ChallengeCard({
  ...
  return (
    <Card>
      <CardHeader>
        <h3 className="text-base font-semibold text-[hsl(var(--charcoal))]">
          {title}
        </h3>
      </CardHeader>
      <CardBody>
...
export function TaskCard({
  ...
  return (
    <Card className="relative">
      <CardBody>
        <p className="text-sm font-medium text-[hsl(var(--charcoal))]">
          {title}
        </p>
        <div className="mt-2 flex items-center gap-2 text-xs text-[hsl(var(--charcoal))]/70">
          {tag ? (
            <span className="rounded-full bg-white px-2 py-0.5">{tag}</span>
          ) : null}
          {due ? <span>Due {due}</span> : null}
        </div>
        <span className="absolute right-2 top-2 cursor-grab select-none text-[hsl(var(--charcoal))]/40">
          ⋮⋮
        </span>
      </CardBody>
    </Card>
  ...
  return (
    <Card>
      ...
      >
        ...
        <path
          d="M0 160 C80 120, 160 140, 240 90 S400 110, 600 60"
